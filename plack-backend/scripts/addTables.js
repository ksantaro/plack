        ...
        // let promiseSetTables = new Promise(function(resolve, reject) {
            client.connect((err)=> {
                if(!err){
                    ...
                    client.query('CREATE TABLE IF NOT EXISTS direct_messages(ufid INT REFERENCES friends, text VARCHAR(300), date timestamp default now(), senderid INT REFERENCES users)');
                    client.query('CREATE TABLE IF NOT EXISTS channel_messages(chid INT REFERENCES channels, text VARCHAR(300), date timestamp default now(), senderid INT REFERENCES users)')
                        .then(res => {
